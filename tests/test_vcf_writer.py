...
class TestVcfWriter:
    ...
    def test_write_w_variants_headers(self, input_file, expected_output_file, tmp_path):
    ...
    def test_write_w_variants(self, input_file, expected_output_file, tmp_path):
        output_path = tmp_path / "output.vcf"
        with VcfReader(input_file) as reader, VcfWriter(output_path) as writer:
    ...
    def test_write_w_headers(self, input_file, expected_output_file, tmp_path):
        output_path = tmp_path / "output.vcf"
        with VcfReader(input_file) as reader, VcfWriter(output_path) as writer:
    ...
    def test_write_filetypes(self, tmp_path, output_suffix, input_file, write_headers, expected_output_file):
        output_file_path = tmp_path / ('output' + output_suffix)
        with VcfReader(input_file) as reader, VcfWriter(output_file_path) as writer:
            writer.write(headers=reader.headers, variants=reader)
        assert open_file(output_file_path).read(9999999).encode().strip() == Path(expected_output_file).read_bytes().strip()

    @pytest.mark.parametrize("input_file, expected_output_file", (
            ('tests/samples/sample1.vcf', 'tests/samples/expected_info_headers_before_last_line.vcf'),
    ))
    def test_write_non_sorted_headers_at_the_end(self, input_file, expected_output_file, tmp_path):
        output_file_path = tmp_path / "output.vcf"
        with VcfReader(input_file) as reader, VcfWriter(output_file_path) as writer:
            new_headers = [
        ...
        assert output_file_path.read_bytes().strip() == Path(expected_output_file).read_bytes().strip()

    @pytest.mark.parametrize("input_file, expected_output_file", (
            ('tests/samples/sample1.vcf', 'tests/samples/expected_info_headers_after_first_line.vcf'),
    ))
    def test_write_non_sorted_headers_at_the_beginning(self, input_file, expected_output_file, tmp_path):
        output_file_path = tmp_path / "output.vcf"
        with VcfReader(input_file) as reader, VcfWriter(output_file_path) as writer:
            new_headers = [
                '##INFO=<ID=tier1_rank,Number=1,Type=Integer,Description="AI rank for tier 1">',
